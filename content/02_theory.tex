...
A distinction can be made between three types of ordered states corresponding to different signs of $J_{ij}$ \lit{Stöhr}.
...
This parameter is called Néel-vector and is defined as the difference between the respective magnetizations of the two sublattices \lit{Stöhr}
\begin{equation}
...
Initially, the ferromagnetic domains are explained as the reason behind their existence is commonly known \lit{Tanner}.
...
So the domain size is mostly determined by the balance of the field energy saved by domain splitting and the energy cost of additional domain walls \lit{Cullity}.
...
This introduces two additional energetic costs to the equation \lit{Feynman}.
...
For a multielectron ion such as Ni$^{2+}$ with a relatively weak spin-orbit coupling all the angular momenta of each electron couple via the Russel-Saunders coupling \lit{Ogale}.
...
In NiO the exchange and crystal splitting are of similar size and the electrons would occupy a high-spin state as shown in \autoref{fig:2}b with $S=1$. But even in the case that $\Delta_{\text{CF}}$ would slightly exceed $\Delta_{\text{Ex}}$ the d$^8$-electrons would still occupy the same ground state \lit{Fromme}.
...
\begin{figure}[ht]
    ...
    \end{subfigure}
    \vspace{-0.1cm}
    \caption{\textbf{(a)} Crystal and magnetic structure of NiPS3. The blue and red arrows at the Ni-atoms depict the AFM coupled spins. \textbf{(b)} The absorption spectrum of NiPS3, showing a d-d transition around \qty{1.1}{eV}. Both taken from \lit{Belvin}.}
\end{figure}
\FloatBarrier
...
A two-lattice model is best used to describe the two magnon modes at the Brioullin-zone center with frequencies of \qty{1.07}{THz} (hf) and \qty{0.14}{THz} (lf) \lit{Rezende} which are visible when pumping with light in the infrared region \lit{Tzschaschel}.
...
Apparently, the magnetic dipole-dipole interaction is responsible for the fact that the degeneracy in the magnon dispersion at the zone-center is lifted and that it is possible to observe two magnon modes. \lit{Gumarilang}
...
\begin{figure}[htb!]
    ...
    \end{subfigure}
    \vspace{-0.3cm}
    \caption{\textbf{(a)} Depiction of the lf- and hf-modes in a given domain. The (111)-plane is highlighted in green. \textbf{(b)} The spectral dependence of the magnon modes in a \qty{100}{\um}-thick NiO crystal measured in transmission. The dependecies are normalized to their highest values. Both taken from \lit{Bossini}.}
\end{figure}
\FloatBarrier
\autoref{fig:7} shows the spectral dependence of the amplitudes of both modes which was measured in transmission on a \qty{100}{\um}-thick NiO crystal \lit{Bossini}.
...
This broadening is explained in terms of the Impulsive Stimulated Raman Scattering (ISRS) \lit{Tzschaschel} which makes it possible to excite magnon modes off-resonantly \lit{2Bossini and Rasing}.
...
Bossini et. al. gave a phenomenological model where the coupling of the two magnon modes was mediated by domain wall oscillations \lit{Bossini}.
...
They are shown in the absorption spectrum in \autoref{fig:7}. and pumping them induces the X-M transition \lit{Bossini}.